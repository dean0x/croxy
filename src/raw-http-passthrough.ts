 ...
 * ADR-010 prohibits.
 */
export const RESPONSE_STRIP = new Set([...HOP_BY_HOP, SYNTHESIZED_HEADER]);
 ...
 */
export const filterRawHeaders = (rawHeaders: readonly string[], strip: ReadonlySet<string>): string[] => {
  const connectionFields = new Set<string>();
  for (let i = 0; i + 1 < rawHeaders.length; i += 2) {
    if (rawHeaders[i]!.toLowerCase() === "connection") {
      for (const token of rawHeaders[i + 1]!.split(",")) connectionFields.add(token.trim().toLowerCase());
    }
  }
...
export const setRawRequestHeaders = (request: http.ClientRequest, rawHeaders: readonly string[]): void => {
  ...
  for (let i = 0; i + 1 < rawHeaders.length; i += 2) {
    ...
    if (entry === undefined) headerMap.set(key, { name, values: [value] });
    else entry.values.push(value);
  }
  for (const { name, values } of headerMap.values()) {
    request.setHeader(name, values.length === 1 ? values[0]! : values);
  }
};
...
export interface PassthroughOptions {
  ...
  /** Maximum sockets in the keep-alive pool for this upstream. */
...
export const createRawHttpForwarder = (options: PassthroughOptions): RawHttpForwarder => {
  ...
  const forward = (req: IncomingMessage, res: ServerResponse, body?: ForwardedBody, requestPath?: string, rawHeaders?: readonly string[]): void => {
    ...
    const filteredRaw = filterRawHeaders(rawHeaders ?? req.rawHeaders, HOP_BY_HOP);
    setRawRequestHeaders(upstream, filteredRaw);
    ...
    upstream.on("timeout", () => {
      if (!settle()) return;
      options.logger.log("warn", options.events.timeout, { path: options.logPath(req) });
      if (!res.headersSent) {
        res.writeHead(504, { "content-type": "application/json", [SYNTHESIZED_HEADER]: SYNTHESIZED_MARKER });
        res.end(options.errorBody("upstream timed out"));
      } else {
        res.destroy();
      }
      if (streaming) {
    ...
    upstream.on("error", () => {
      if (!settle()) return;
      options.logger.log("warn", options.events.error, { path: options.logPath(req) });
      if (!res.headersSent) {
        res.writeHead(502, { "content-type": "application/json", [SYNTHESIZED_HEADER]: SYNTHESIZED_MARKER });
        res.end(options.errorBody("upstream connection failed"));
      } else {
        res.destroy();
      }
      if (streaming) {
