...
export class ClaudeHttpError extends Error {
  constructor(readonly status: number, message: string, readonly code: string, readonly retryAfter?: string) { super(message); }
}

export class ClaudeHandler {
  private readonly state: ReverseState;
  constructor(private readonly config: ClaudeProviderConfig, private readonly auth: ClaudeAuth, private readonly logger: Logger,
    private readonly fetchImpl: typeof fetch = fetch) { this.state = new ReverseState(undefined, config.continuationCache); }
  ...
  async *respond(request: Item, externalSignal: AbortSignal, sessionKey?: string): AsyncGenerator<Item> {
    ...
    let terminal = false;
    const started = Date.now(), model = String(request["model"]);
    const trace = sessionKey === undefined ? {} : { sessionKey };
    try {
      const translated = reverseRequest({ ...request, max_output_tokens: request["max_output_tokens"] ?? 64000 }, [{ type: "text", text: CLAUDE_SUBSCRIPTION_PREAMBLE }], this.state);
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        const headers = { ...credentials.value.authHeaders, "content-type": "application/json", accept: "text/event-stream",
          "anthropic-version": "2023-06-01", "anthropic-beta": "claude-code-20250219,oauth-2025-04-20", "user-agent": `${SUBSWITCH_NAME}/${SUBSWITCH_VERSION}` };
        response = await this.fetchImpl(`${this.config.baseUrl.replace(/\/$/, "")}/v1/messages`, {
          method: "POST", headers, body: JSON.stringify({ ...translated.body, stream: true }), redirect: "error", signal,
        });
        if (response.status !== 401 || attempt === 1) break;
        await response.body?.cancel();
        this.logger.log("info", "claude_upstream_401_refreshing", { model, ...trace });
      ...
      if (!response.ok) {
        let message = "Claude rejected the request.", code = "claude_upstream_error";
        ...
        } catch { /* Never return upstream HTML or parser exceptions. */ }
        const token = credentials.ok ? credentials.value.authHeaders["authorization"]?.replace(/^Bearer /, "") : undefined;
        message = redactCredentials(token ? message.replaceAll(token, "<redacted>") : message);
        this.logger.log("warn", "claude_upstream_error", { model, ...trace, status: response.status, errorCode: code });
      ...
      const resetIdle = () => { clearTimeout(idle); idle = setTimeout(() => controller.abort(), this.config.streamIdleTimeoutMs); };
      source.on("data", resetIdle); resetIdle();
      const running = pipeline(source, parser, { signal }).catch(error => { parser.destroy(error as Error); });
      try {
        for await (const event of translateClaudeStream(parser as AsyncIterable<SseEvent>, {
          id: `resp_subswitch_${randomUUID()}`, model, request: translated, state: this.state, maxBytes: this.config.maxAggregateBytes,
        })) {
          if (event["type"] === "response.output_item.done" && ["function_call", "custom_tool_call"].includes(String(object(event["item"])?.["type"])))
            this.logger.log("info", "claude_tool_call", { model, ...trace });
          ...
          if (terminal) {
            ...
            this.logger.log("info", "claude_request_complete", { model, ...trace, status: 200, latencyMs: Date.now() - started,
        ...
        await running;
      } finally { parser.destroy(); await running; }
    } catch (error) {
      if (terminal && signal.aborted) return;
      if (signal.aborted && !externalSignal.aborted) throw new ClaudeHttpError(504, "Claude request timed out.", "claude_timeout");
      throw error;
    } finally {
      clearTimeout(timeout); clearTimeout(idle); source?.destroy();
    }
  }
}
