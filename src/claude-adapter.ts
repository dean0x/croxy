...
const fail = (code: string): never => { throw new ReverseContractError(code); };
...
export interface ToolMapping { wire: string; name: string; namespace?: string; type: "function" | "custom"; definition: Item }
export interface ReverseRequest { body: Item; tools: ReadonlyMap<string, ToolMapping> }

export function toolWireName(namespace: string | undefined, name: string, type: string): string {
  return `ss_${createHash("sha256").update(JSON.stringify([namespace ?? null, name, type])).digest("hex").slice(0, 32)}`;
}
...
function content(value: unknown, toolResult = false): Item[] {
  ...
  return value.map(value => {
    ...
    if (["input_text", "output_text", "text"].includes(String(block["type"]))) return { type: "text", text: string(block["text"]) };
...
export function reverseRequest(request: Item, nativePreamble: readonly Item[] = [], state?: ReverseState): ReverseRequest {
  const input = typeof request["input"] === "string" ? [{ type: "message", role: "user", content: request["input"] }] :
    Array.isArray(request["input"]) ? request["input"].map(item) : fail("unsupported_input");
  ...
  const register = (definition: Item, namespace?: string): void => {
    ...
    const name = string(definition["name"]), wire = toolWireName(namespace, name, type);
    const existing = tools.get(wire);
    if (existing && JSON.stringify(existing.definition) !== JSON.stringify(definition)) return fail("tool_definition_conflict");
  ...
  for (const entry of input) if (entry["type"] === "additional_tools") {
  ...
  const lookup = (entry: Item, type: "function" | "custom"): ToolMapping => {
  ...
  for (let inputIndex = 0; inputIndex < input.length; inputIndex++) {
    ...
    if (type === "reasoning" && state) {
      ...
      if (following.some(value => value["encrypted_function_args"] !== undefined &&
        (!Array.isArray(value["encrypted_function_args"]) || value["encrypted_function_args"].length))) return fail("encrypted_tool_arguments");
      if (following.length !== replay.output.length || following.some((value, index) => replayIdentity(value) !== replayIdentity(replay.output[index]!))) {
        return fail("state_history_mismatch");
      }
      for (const block of replay.content) if (block["type"] === "tool_use") {
        const id = string(block["id"]); if (calls.has(id)) return fail("duplicate_tool_call"); calls.add(id);
      }
    ...
    if (type === "message" || (type === undefined && entry["role"] !== undefined)) {
      ...
      const blocks = content(entry["content"]);
      append("user", [{ type: "text", text: `Agent message from ${string(entry["author"])} to ${string(entry["recipient"])}:` }, ...blocks]);
    } else if (type === "function_call" || type === "custom_tool_call") {
      if (entry["encrypted_function_args"] !== undefined && (!Array.isArray(entry["encrypted_function_args"]) || entry["encrypted_function_args"].length))
        return fail("encrypted_tool_arguments");
      const mapping = lookup(entry, type === "function_call" ? "function" : "custom");
      const id = string(entry["call_id"]);
      if (calls.has(id)) return fail("duplicate_tool_call");
      calls.add(id);
      let args: Item;
      try { args = type === "custom_tool_call" ? { input: string(entry["input"]) } : item(JSON.parse(string(entry["arguments"]))); }
      catch { return fail("invalid_tool_arguments"); }
    ...
    } else return fail(type === "reasoning" || type === "compaction" ? "opaque_state_unimplemented" : "unsupported_input_item");
  }
  if ([...calls].some(id => !results.has(id))) return fail("missing_tool_result");
  const mappedTools = [...tools.values()].map(mapping => {
    const definition = mapping.definition;
    const description = definition["description"] === undefined ? "" : string(definition["description"]);
    return { name: mapping.wire,
      description: mapping.type === "custom" ? `${description}\nReturn the original freeform tool input verbatim in the input string. Tool format: ${JSON.stringify(definition["format"] ?? { type: "text" })}` : description,
      input_schema: mapping.type === "custom" ? { type: "object", properties: { input: { type: "string" } }, required: ["input"], additionalProperties: false } :
  ...
  if (request["tool_choice"] === "none") choice = { type: "none" };
    ...
    choice = { type: "tool", name: lookup(selected, selected["type"]).wire };
  } else if (request["tool_choice"] !== undefined && request["tool_choice"] !== "auto") return fail("unsupported_tool_choice");
  ...
  if (effort !== undefined && !["none", "low", "medium", "high", "xhigh", "max"].includes(String(effort))) return fail("unsupported_reasoning_effort");
  return { tools, body: { model: string(request["model"]), max_tokens: max, stream: false,
    cache_control: { type: "ephemeral" },
    ...(system.length ? { system } : {}), messages,
    ...(mappedTools.length ? { tools: mappedTools, tool_choice: choice } : {}),
    ...(effort === "none" ? { thinking: { type: "disabled" } } : effort !== undefined ? { thinking: { type: "adaptive" }, output_config: { effort } } : {}),
  } };
}

/** Completed JSON only. Partial/truncated arguments never become executable native calls. */
export function reverseResponse(response: Item, request: ReverseRequest, state?: ReverseState, identities?: ReadonlyMap<number, string>, stateToken?: string): Item[] {
  if (response["type"] !== "message" || !Array.isArray(response["content"])) return fail("invalid_claude_response");
  if (response["stop_reason"] !== "end_turn" && response["stop_reason"] !== "tool_use") return fail("incomplete_claude_response");
  const output: Item[] = [];
  const callIds = new Set<string>();
  for (const [blockIndex, value] of response["content"].entries()) {
    const block = item(value);
    if (block["type"] === "text") {
      string(block["text"]); output.push({ type: "message", id: identities?.get(blockIndex) ?? `msg_${randomUUID()}`, role: "assistant", phase: response["stop_reason"] === "tool_use" ? "commentary" : "final_answer", status: "completed",
        content: [{ type: "output_text", text: block["text"], annotations: [] }] });
    } else if (block["type"] === "tool_use") {
      const mapping = request.tools.get(string(block["name"])) ?? fail("unknown_claude_tool");
      const args = item(block["input"]), callId = string(block["id"]);
      if (!callId || callIds.has(callId)) return fail("duplicate_or_missing_tool_call_id");
      callIds.add(callId);
      const common = { id: identities?.get(blockIndex) ?? `fc_${randomUUID()}`, call_id: callId, name: mapping.name, status: "completed",
      ...
      if (mapping.type === "custom") {
        ...
        output.push({ ...common, type: "custom_tool_call", input: string(args["input"]) });
      } else output.push({ ...common, type: "function_call", arguments: JSON.stringify(args),
  ...
  if (state) {
    output.unshift({ type: "reasoning", id: `rs_${randomUUID()}`, summary: [],
...
export function reverseEvents(id: string, model: string, output: Item[], usage: Item = {}): Item[] {
  ...
  const frames: Item[] = [{ type: "response.created", response: base }, { type: "response.in_progress", response: base }];
  output.forEach((entry, output_index) => {
    const type = entry["type"], item_id = entry["id"];
    frames.push({ type: "response.output_item.added", output_index, item: { ...entry, status: "in_progress",
      ...(type === "function_call" ? { arguments: "" } : type === "custom_tool_call" ? { input: "" } : { content: [] }) } });
    if (type === "message") {
      const part = (entry["content"] as Item[])[0]!;
      const info = { item_id, output_index, content_index: 0 };
      frames.push({ type: "response.content_part.added", ...info, part: { ...part, text: "" } },
      ...
      const custom = type === "custom_tool_call", field = custom ? "input" : "arguments";
      const stem = custom ? "response.custom_tool_call_input" : "response.function_call_arguments";
      frames.push({ type: `${stem}.delta`, item_id, output_index, delta: entry[field] },
  ...
  frames.push({ type: "response.completed", response: { ...base, status: "completed", output,
